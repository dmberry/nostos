...
// #145 V1b — the reusable canvas console.
//
// The estate's terminals are DOM overlays, which is correct for a full-screen
// console but wrong for one that has to sit BEHIND a window on her Workspace: a
// DOM layer is always above the canvas. This module is the island-agnostic
// model of a console rendered ON the canvas — scrollback, one editable input
// line, history, and word-wrap to a column width. It is deliberately pure: it
// holds text and a cursor and nothing else, so the whole thing is testable in
// node and can drive CALYPSO's Terminal now and any other island's console
// later, each with its own theme and its own command dispatch.
//
// It does NOT run commands. The host calls submit() to take the typed line,
// runs it however that station runs it (coreRun, the laptop shell, whatever),
// and calls print() with the result. Keeping dispatch outside is what makes one
// buffer serve every console.
