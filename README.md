...
### Recent (v1.176 … v1.165)

| Version | Summary |
|---|---|
| v1.176 | **Dead blight looks dead, and the goggles are night-vision.** Two fixes from playtest. The dead stage of the blight was still grey *grass* — blades and all — which read as lawn, not death. It is now **bare ashen earth**: no grass blades on the fully-dead tiles (they stay on the sickly-yellow front, dying grass), a darker drained colour, and a faint dark rot-mottle over it. And the **night-vision goggles now lift the night**, not only POSEIDON's fog — they were doing nothing if you tried them in the dark, which is exactly what a player would test. Worn, the dark veil and the fog both give way to a **green phosphor view with scanlines** (screen-blended, so it lifts the darkness rather than tinting over it), a little stronger the darker or foggier it would otherwise be. Verified both in the browser. |
| v1.175 | **POSEIDON's fog, night-vision goggles, and the blight as recoloured grass.** Three connected pieces of the POSEIDON-with-teeth work. **The fog:** when the network wakes it drags a grey veil over the island, collapsing sight to a short pool around you; its density tracks how much of the network still stands, so it slams down at activation and **disperses slowly as you fell towers**, gone when the network is broken. **Night-vision goggles** are the one thing that pierces it — found, or crafted from **5 torches + a circuit board** (another circuit sink), clicked in any slot to wear. With them on the veil gives way to a thin green wash you can see through. And the **blight now recolours the grass tile** rather than swapping in a flat grey: it draws the grass texture and blades tinted by its stage colour — sickly yellow at the front, drained grey behind — so it reads as the living ground *sickening*, same blades, not a blob. Verified in the browser: the green→yellow→grey gradient with the texture intact, the fog veil and its clear pool, and the goggles craft consuming its ingredients. |
| v1.174 | **POSEIDON's blight — the land turns to standing reserve.** The first slice of the POSEIDON-with-teeth work (docs/poseidon-consequences-plan.md). When the network wakes, a **blight spreads outward from every live obelisk**, tile by tile: the grass yellows and sickens at the leading edge and drains to dead grey behind it, and any **tree standing on blighted ground dies** (its canopy greys). Each tower owns its own front — and **killing or jamming a tower freezes ITS front and lets that ground recover**, green returning around the wreck. So felling a tower is watched, not abstract: the sum of how many fronts you have stopped is how much island is left. Pure spread arithmetic in `game/blight.js` (8 tests: each tower's front is independent, a felled tower's ground comes back, jam/freeze halt it too), applied to the floor grid in the hub. Only the living ground blights (sea, road, decks and mountain rock are left). Also: the default character name is now **Nobody** — Odysseus's *Outis* to the Cyclops, and the `nobody` the obelisk terminals already accept. |
| v1.173 | **The weapon swings in place instead of flying away.** On use, a melee tool's draw position was pushed outward by up to ~2.3x its rest distance (`reach = 0.42 + pulse * 0.55`), so the weapon lunged off into the empty space beside the character — odd and unnatural. The swing is now a **rotation about the hand**: the reach stays put (a hair of forward thrust only), the arc is all in the angle, and a grip offset makes the blade sweep from its handle rather than spin on its centre. It reads as a real swing now. |
| v1.172 | **Fix: a swing at empty air now animates.** Testers pressed the use key with nothing in range and, seeing the tool not move, read the controls as broken. The whole swing arc is driven by `swingTimer` (`renderer.drawHeldItem`), and the empty-air melee branch played a *sound* but never set it — so a whiff was invisible. It now sets `swingTimer` (the arc sweeps) and costs **half stamina** (you swung, but hit nothing to follow through on), never health. Guns were already fine: they set the timer for recoil and spawn a visible tracer whether or not they hit. So the tool always moves on press/click now, which is the feedback a new player needs to learn the controls. |
